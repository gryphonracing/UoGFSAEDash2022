...
class UrlInterceptor : public QQmlAbstractUrlInterceptor {
  public:
    ...
    QUrl intercept(QUrl const& url, QQmlAbstractUrlInterceptor::DataType /*type*/) final {
        ...
        if ( ! url_str.startsWith(QStringLiteral("qrc")) ||
        url_str.endsWith(QStringLiteral("qmldir"))) return url; qCDebug(log, "Intercepting: %s",
        qPrintable(url_str));//*/

        if (filesMap.count(url) > 0) {
            // qCDebug(log, "Intercepted: %s to %s", qPrintable(url.toString()),
            // qPrintable(filesMap[url].toString()));
            return filesMap[url];
        }
        return url;
    }
    ...
    void addUrlMap(QUrl const& source, QUrl const& target) {
        // qCDebug(log, "Intercepting: %s to %s", qPrintable(source.toString()),
...
class RuntimeQmlPrivate {
    ...
    RuntimeQmlPrivate(RuntimeQml* q, QQmlApplicationEngine* _engine) : q_ptr(q), engine(_engine) {
        // The URL interceptor is used to replace QRC files by filesystem files.
        engine->addUrlInterceptor(&urlInterceptor);

        QObject::connect(
            &fileWatcher, &QFileSystemWatcher::fileChanged, q_ptr, [&](QString const& path) {
                if (!autoReload)
                    return;
                ...
                QTimer::singleShot(
                    500, &fileWatcher, [this, path]() { fileWatcher.addPath(path); });
#endif
            });
    }
    ...
    //! Process QRC entries: add filtered files to watcher and URL interceptor
    {
        ...
        if (!file.open(QIODevice::ReadOnly | QIODevice::Text)) {
            qCWarning(log,
                      "Unable to open resource file '%s'. Error: %s",
                      qPrintable(qrcFilename),
            ...
            if (inputStream.isStartElement()) {
                ...
                if (name == QStringLiteral("qresource")) {
                    if (inputStream.attributes().hasAttribute(QStringLiteral("prefix"))) {
                        currentPrefix =
                            inputStream.attributes().value(QStringLiteral("prefix")).toString();
                ...
                // Check file name
                    ...
                    bool const toIgnore =
                        std::find_if(fileIgnoreList.cbegin(),
                                     fileIgnoreList.cend(),
                                     [&](QString const& pattern) {
                                         QRegularExpression re(
                                             wildcardToRegularExpression(pattern));
...
//! Applies to the full prefix+filename in the QRC entry (i.e. the QRC path, not the local
//! filesystem path). Must be called before load() and parseQrc().
{
    Q_D(RuntimeQml);
...
//! @brief Add a QRC prefix to ignore. Equivalent to addIgnoreFilter("qrc:/<prefix>") for a valid
//! prefix. Must be called before load() and parseQrc().
{
    if ((!prefix.isEmpty()) && (!prefix.startsWith(QStringLiteral("qrc:/"))) &&
