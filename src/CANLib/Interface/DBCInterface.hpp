...
class DBCInterface : public CAN::Interface {
  public:
    DBCInterface(const char* dbc_file) : dbc_network(std::move(dbcppp::INetwork::LoadNetworkFromFile(dbc_file)[""])) {
        this->CAN::Interface::startReceiving("can0",
                                             CANNode::filters,
                                             CANNode::num_of_filters,
                                             CANNode::timeout_ms);
  ...
  private:
    ...
    void newFrame(const can_frame& frame) override {
        ...
        for (const dbcppp::ISignal& sig : message->second->Signals()){
            ...
            std::invoke(signal_dispatch_func_iter->second, dynamic_cast<CANNode*>(this), sig.RawToPhys(sig.Decode(frame.data))); // Send the data to the front end
        }
    }
